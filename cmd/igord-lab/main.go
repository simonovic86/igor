...
// igord-lab is the research/P2P CLI for Igor. It preserves the legacy flag-based
// interface with full migration, replay verification, lease management, and
// pricing support. For the product CLI, use igord instead.
package main
...
func main() {
	...
	logger.Info("Igor Lab Node starting...")
	...
	logger.Info("Igor Lab Node ready")
	...
	logger.Info("Igor Lab Node shutting down...")
}
...
func handleTick(
	...
	if leaseErr := research.CheckAndRenewLease(instance, logger); leaseErr != nil {
		...
		return tickNormal, research.HandleLeaseExpiry(ctx, instance, leaseErr, logger)
	}
	...
	*ticksSinceVerify++
	if periodicVerify && cfg.VerifyInterval > 0 && *ticksSinceVerify >= cfg.VerifyInterval {
		...
		*lastVerifiedTick, action = research.VerifyNextTick(ctx, instance, replayEngine, *lastVerifiedTick, cfg.ReplayCostLog, cfg.ReplayOnDivergence, logger)
		if stop := research.HandleDivergenceAction(ctx, instance, cfg, action, nil, logger); stop {
